...
class LookupModule(LookupBase):
    ...
    def run(self, terms, inject=None, variables=None, **kwargs):
        ...
        vault_token = os.getenv('VAULT_TOKEN') or (variables or inject).get('vault_token')
    ...
    def _fetch_token(self, cafile, capath, github_token, url, cahostverify):
        try:
            ...
            raise AnsibleError('Unable to retrieve personal token from vault1: %s' % (e))
        except urllib2.HTTPError as e:
            raise AnsibleError('Unable to retrieve personal token from vault2: %s' % (e))
        except Exception as e:
            raise AnsibleError('Unable to retrieve personal token from vault3: %s' % (e))
    ...
    def _fetch_remotely(self, cafile, capath, data, key, vault_token, url, cahostverify):
        try:
            ...
            raise AnsibleError('Unable to read %s from vault4: %s' % (key, e))
        except urllib2.HTTPError as e:
            raise AnsibleError('Unable to read %s from vault5: %s' % (key, e))
        except Exception as e:
            raise AnsibleError('Unable to read %s from vault6: %s' % (key, e))
        result = json.loads(response.read())
        return result
